    ...
    const renderRatingChange = (row) => {
        ...
        return (
            <Grid container justifyContent="left">
                {diff <= 0 ?  <ArrowDownward item style={{color: 'red'}}/> : <ArrowUpward item style={{color: 'green'}}/>}
                <Typography item style={{color: diff <= 0 ? 'red' : 'green'}}>
                    {diff}
                </Typography>
            </Grid>
        )
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              ...
              <TableBody>
                {stableSort(rows, getComparator(order, orderBy)).map((row) => (
                  ...
                  >
                    <TableCell component="th" scope="row">{row.contestId}</TableCell>
